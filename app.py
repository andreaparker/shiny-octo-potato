...
# users = {
#     'admin': {
#         'password': generate_password_hash('sl-s3cr3t-upwards')  # Replace with your desired password
#     }
# }
...
# # Flag to toggle password-protected login
# DISABLE_PASSWORD = os.getenv('DISABLE_PASSWORD', 'false').lower() == 'true'
...
#         if user_record:
#             # Check if password is disabled or validate the password
#             if DISABLE_PASSWORD or check_password_hash(user_record['password'], password):
#                 user = User(username)
#                 login_user(user)
...
#                     index_name = session_id
#                     index_path = os.path.join(app.config['INDEX_FOLDER'], index_name)
#                     indexer_model = session.get('indexer_model', 'vidore/colpali-v1.2')
#                     RAG = index_documents(session_folder, index_name=index_name, index_path=index_path, indexer_model=indexer_model)
#                     if RAG is None:
#                         raise ValueError("Indexing failed: RAG model is None")
#                     RAG_models[session_id] = RAG
...
#     model_choice = session.get('model', 'gpt4') ## used to be qwen
#     resized_height = session.get('resized_height', 280)
#     resized_width = session.get('resized_width', 280)
...
# def settings():
#     if request.method == 'POST':
#         indexer_model = request.form.get('indexer_model', 'vidore/colpali-v1.2')
